...
u64 CalcOffset::next(u64 s) {
    ++offset;
    return lcg.next(s);
}

u64 CalcOffset::execute_pt(u64 s) {
    u64 r;
    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 5; j++) {
...
u64 CalcOffset::execute_extra(u64 s) {
    u64 r1,r2,r3;
...
u64 CalcOffset::get_seed() const { return seed; }
u64 CalcOffset::get_offset() const { return offset; }

u64 CalcOffset::bw1(u64 seed_, bool has_continued_, bool exist_data_) {
    seed = seed_;
    offset = 0;
    if(has_continued_) {
        seed = execute_pt(seed);
        seed = execute_pt(seed);
        seed = execute_pt(seed);
        seed = execute_pt(seed);
        seed = execute_pt(seed);
